...
def synthetic_feed(hour_local: int = 17, seed: int = 42, now_epoch: float | None = None):
    """
    Build one moment of plausible feed data, shaped exactly like the decoded
    protobuf output so tests exercise the real aggregation path.

    Returns (vehicles, predictions, alerts, routes).

    The predictions are generated by simulating vehicles departing a terminal at
    a target headway and then drifting: a bus that falls behind picks up more
    passengers and falls further behind, which is why real transit bunches. The
    drift is scaled by time of day so the PM peak produces visibly worse
    regularity than 3am - meaning the fixtures produce a metric that actually
    varies, rather than a flat number that would hide a broken calculation.
    """
    ...
    for rid, meta in ROUTES.items():
        ...
        n_veh = max(2, int(rng.gauss(base, 2)))
        for i in range(n_veh):
            vehicles.append({"route_id": rid, "vehicle_id": f"{rid}-{i}",
                             "lat": 43.65 + rng.random() * 0.1, "lon": -79.4 + rng.random() * 0.1})

        # Target headway: frequent on the spines, sparse on the tail.
        target = 60 * (6 if rid in dict(BUS_SPINES) or meta["mode"] == "streetcar" else 18)
        # Streetcars in mixed traffic bunch harder than suburban buses.
        chaos = (0.45 if meta["mode"] == "streetcar" else 0.28) * scale

        n_stops = 12
        # Each vehicle's accumulated lateness, which compounds along the route.
        offsets = [rng.gauss(0, target * chaos) for _ in range(n_veh)]
        for stop_i in range(n_stops):
            stop_id = f"{rid}-S{stop_i}"
            for v in range(n_veh):
                offsets[v] += rng.gauss(0, target * chaos * 0.35)
                t = now + 120 + v * target + stop_i * 95 + offsets[v]
                if now - 60 <= t <= now + 3600:
                    predictions.append({"route_id": rid, "stop_id": stop_id,
                                        "trip_key": f"{rid}-t{v}", "time": round(t)})

    alerts = [{"route_ids": [rid], "header": "Detour in effect"}
              for rid in rng.sample(list(ROUTES), 3)]

    return vehicles, predictions, alerts, ROUTES
...
def seed_demo_history(days: int = 7, every_minutes: int = 10) -> None:
    ...
    for i in range(steps, 0, -1):
        ...
        vehicles, preds, alerts, routes = synthetic_feed(
            local_hour, seed=i, now_epoch=ts_dt.timestamp())
        rows = build_snapshot(iso(ts_dt), vehicles, preds, alerts, routes,
                              now_epoch=ts_dt.timestamp())
...
if __name__ == "__main__":
    ...
    if args.seed_demo:
        ...
        print(f"{len(v)} vehicles, {len(t)} stop predictions, {len(a)} alerts, {len(r)} routes")
